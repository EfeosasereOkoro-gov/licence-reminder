...
export interface ExtractProgress {
  /** Short message describing what we're doing right now. */
  message: string;
  /** 0–1 fraction. Combined across engine load and recognition. */
  progress: number;
}
...
// Use the "fast" tessdata variant — ~3 MB vs ~10 MB for "best", and
// noticeably quicker OCR. Driving-licence text is clean sans-serif, so
// the accuracy hit is acceptable for the prototype.
const TESS_LANG_PATH = 'https://tessdata.projectnaptha.com/4.0.0_fast';

let workerPromise: Promise<Worker> | null = null;

/**
 * Start downloading and initialising the OCR engine. Safe to call early
 * (e.g. when the photo panel opens) — subsequent calls return the same
 * worker.
 */
export function prewarmExtractor(
  ...
  workerPromise = (async () => {
    ...
    const worker = await createWorker('eng', 1, {
      ...
      // Tesseract.js v6+ emits objects like { status, progress } here.
      logger: (m: { status: string; progress: number }) => {
        // Scale the warmup phases into 0–0.6; OCR will take 0.6–1.0.
        const label =
 ...
 * Phone photos are commonly 4000+ pixels on a side; OCR time scales
 * roughly with pixel count, so this is one of the biggest wins.
 */
async function shrinkForOCR(file: File, maxDim = 1280): Promise<Blob> {
  ...
  // OffscreenCanvas keeps the work off the main thread where available.
...
function findExpiryDate(text: string): { day: number; month: number; year: number } | null {
  ...
  // Fallback — latest plausible date in the document.
